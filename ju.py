...
def findword(query):
    ...
    for w in words:
        ...
        if not (w in history):           
            ...
            if len(word) == 3 and pos == '명사' and not word in history and not word[len(word)-1] in blacklist:
                ans.append(w)
    if len(ans)>0:
...
def checkexists(query):
    ...
    for w in words:
        ...
        if not (w in history):           
            ...
            word = midReturn(w,'<word>','</word>')
            pos = midReturn(w,'<pos>','</pos>')
            if len(word) == 3 and pos == '명사' and word == query: ans = w
...
def get_chosung(word):
    return ''.join([hgtk.letter.decompose(c)[0] for c in word])
...
while(playing):
    ...
    if answord:
        start = answord[-1]
        hint_word = findword(start + '*')
        if hint_word:
            hint = get_chosung(midReturn(hint_word, '<word>', '</word>'))
            print(f"힌트는: {hint}입니다.")
